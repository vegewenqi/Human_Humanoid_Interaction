...
FusionGLViewer* currentInstance_ = nullptr;

FusionGLViewer::FusionGLViewer()
    : available(false) {
...
bool FusionGLViewer::isAvailable() {
    if (available)
        glutMainLoopEvent();
    return available;
}
...
void FusionGLViewer::init(int argc, char** argv) {
    ...
    glutDisplayFunc(FusionGLViewer::drawCallback);
    glutMouseFunc(FusionGLViewer::mouseButtonCallback);
    glutMotionFunc(FusionGLViewer::mouseMotionCallback);
    glutReshapeFunc(FusionGLViewer::reshapeCallback);
    glutKeyboardFunc(FusionGLViewer::keyPressedCallback);
    glutKeyboardUpFunc(FusionGLViewer::keyReleasedCallback);
...
sl::float3 FusionGLViewer::getColor(int id, bool for_skeleton) {
...
void FusionGLViewer::updateCamera(int id, sl::Mat& view, sl::Mat& pc) {
...
void FusionGLViewer::updateCamera(sl::Mat& pc) {
    const std::lock_guard<std::mutex> lock(mtx);
    int id = 0;
    auto clr = getColor(id, false);
...
void FusionGLViewer::setRenderCameraProjection(sl::CameraParameters params, float znear, float zfar) {
...
void FusionGLViewer::setCameraPose(int id, sl::Transform pose) {
    const std::lock_guard<std::mutex> lock(mtx);
    auto clr = getColor(id, false);
    poses[id] = pose;
...
void FusionGLViewer::updateBodies(sl::Bodies& bodies, std::map<sl::CameraIdentifier, sl::Bodies>& singledata, sl::FusionMetrics& metrics) {
    const std::lock_guard<std::mutex> lock(mtx);
    ...
    for (auto& it : singledata) {
        ...
        ObjectClassName obj_str;
        obj_str.name_lineA
            = "CAM: " + std::to_string(it.first.sn) + " FPS: " + std::to_string(metrics.camera_individual_stats[it.first].received_fps);
        obj_str.name_lineB = "Ratio Detection :" + std::to_string(metrics.camera_individual_stats[it.first].ratio_detection) + " Delta "
            + std::to_string(metrics.camera_individual_stats[it.first].delta_ts * 1000.f);
        obj_str.color = clr;
        obj_str.position = sl::float3(10, (id * 30), 0);
        fusionStats.push_back(obj_str);
    }
...
void FusionGLViewer::clearInputs() {
...
void FusionGLViewer::drawCallback() {
    currentInstance_->render();
}

void FusionGLViewer::mouseButtonCallback(int button, int state, int x, int y) {
    if (button < 5) {
        if (button < 3) {
...
void FusionGLViewer::mouseMotionCallback(int x, int y) {
...
void FusionGLViewer::reshapeCallback(int width, int height) {
...
void FusionGLViewer::keyPressedCallback(unsigned char c, int x, int y) {
    currentInstance_->keyStates_[c] = KEY_STATE::DOWN;
    currentInstance_->lastPressedKey = c;
    // glutPostRedisplay();
}

void FusionGLViewer::keyReleasedCallback(unsigned char c, int x, int y) {
    currentInstance_->keyStates_[c] = KEY_STATE::UP;
}

void FusionGLViewer::idle() {
    glutPostRedisplay();
}
