...
def format_summary_text(summary, angle_unit="deg"):
    ...
    lines = [
        r"$M_{\mathrm{clear}}$: "
        f"{summary.get('self_unsafe_M_clear_m', float('nan')):.3f} m → "
        f"{summary.get('self_safe_M_clear_m', float('nan')):.3f} m",
        r"$M_{\mathrm{ctr}}$: "
        f"{100 * summary.get('self_unsafe_M_ctr', float('nan')):.1f}% → "
        f"{100 * summary.get('self_safe_M_ctr', float('nan')):.1f}%",
        r"$M_{\mathrm{cc}}$: "
        f"{summary.get('self_unsafe_M_cc', 'nan')} → "
        f"{summary.get('self_safe_M_cc', 'nan')}",
        r"RMSE$_q^0$: "
...
def plot_main(
    ...
    # Same joint: same color.
    # Nominal: dashed. CBF: solid.
    # ------------------------------------------------------------------
    ax = axes[0]
    ...
    # ax.set_title("Simulation self-collision avoidance")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=7.8, ncol=2, framealpha=0.78, loc="lower right")
    ax.margins(x=0)

    # ------------------------------------------------------------------
    # Panel 2: global minimum collision clearance
    # ------------------------------------------------------------------
    ax = axes[1]
    unsafe_col = "self_unsafe_global_min_clearance"
    safe_col = "self_safe_global_min_clearance"
    ...
    for fmt in formats:
        out = outdir / f"fig_real_self_collision_main.{fmt}"
...
def pretty_pair_name(col):
    s = col
    s = s.replace("self_unsafe_clearance__", "")
    s = s.replace("self_safe_clearance__", "")
    s = s.replace("__", " vs ")
    s = s.replace("_", " ")
    return s
...
def plot_pairs(run_dir: Path, outdir: Path, t_start, t_end, smooth_window, top_k, formats):
    ...
    unsafe_cols = get_top_pair_columns(df, "self_unsafe_clearance__", top_k=top_k)
    safe_cols = [c.replace("self_unsafe_clearance__", "self_safe_clearance__") for c in unsafe_cols]
    ...
    ax.set_title("Most critical self-collision monitored pairs")
    ax.grid(True, alpha=0.3)
    ...
    outdir.mkdir(parents=True, exist_ok=True)
    for fmt in formats:
        out = outdir / f"fig_real_self_collision_pairs.{fmt}"
        fig.savefig(out, dpi=300, bbox_inches="tight")
        print(f"Saved: {out}")
